...
FROM (((ArtenDb_tblFloraSisf INNER JOIN (tblAktionsplan INNER JOIN vApApBerichtRelevant ON tblAktionsplan.ApArtId = vApApBerichtRelevant.ApArtId) ON ArtenDb_tblFloraSisf.NR = tblAktionsplan.ApArtId) LEFT JOIN ArtenDb_tblFloraFnsKef ON tblAktionsplan.ApArtId = ArtenDb_tblFloraFnsKef.SisfNr) LEFT JOIN vFnsJahrespflanzen ON tblAktionsplan.ApArtId = vFnsJahrespflanzen.SisfNr) INNER JOIN (tblKonstanten INNER JOIN (tblApJBer INNER JOIN vApAnzMassnBisJahr ON tblApJBer.ApArtId = vApAnzMassnBisJahr.ApArtId) ON tblKonstanten.ApBerJahr = tblApJBer.ApBerJahr) ON tblAktionsplan.ApArtId = tblApJBer.ApArtId
...
SELECT If(ArtenDb_tblFloraSisf.Deutsch is null,ArtenDb_tblFloraSisf.Name, CONCAT(ArtenDb_tblFloraSisf.Name, ' (', ArtenDb_tblFloraSisf.Deutsch, ')')) AS Artname, vApAnzMassnBisJahr.AnzahlMassnahmen
FROM (ArtenDb_tblFloraSisf INNER JOIN (tblAktionsplan INNER JOIN vApApBerichtRelevant ON tblAktionsplan.ApArtId = vApApBerichtRelevant.ApArtId) ON ArtenDb_tblFloraSisf.NR = tblAktionsplan.ApArtId) INNER JOIN vApAnzMassnBisJahr ON tblAktionsplan.ApArtId = vApAnzMassnBisJahr.ApArtId
WHERE ((vApAnzMassnBisJahr.AnzahlMassnahmen>0) AND (tblAktionsplan.ApStatus Between 1 And 3))
ORDER BY ArtenDb_tblFloraSisf.Name;

CREATE VIEW vJbUebMe AS
SELECT tblApJBer.*, If(ArtenDb_tblFloraSisf.Deutsch is null,ArtenDb_tblFloraSisf.Name, CONCAT(ArtenDb_tblFloraSisf.Name, ' (', ArtenDb_tblFloraSisf.Deutsch, ')')) AS Artname, If("KefArt"=-1,"Ja","") AS FnsKefArt2, If(Round((tblKonstanten.ApBerJahr-"KefKontrolljahr")/4,0)=(tblKonstanten.ApBerJahr-"KefKontrolljahr")/4,"Ja","") AS FnsKefKontrJahr2, vFnsJahrespflanzen.Jahr AS FnsJahrespflanze, vFnsJahrespflanzen.Jahresart AS FnsJahrespflanze2
FROM (((ArtenDb_tblFloraSisf INNER JOIN (tblAktionsplan INNER JOIN vApApBerichtRelevant ON tblAktionsplan.ApArtId = vApApBerichtRelevant.ApArtId) ON ArtenDb_tblFloraSisf.NR = tblAktionsplan.ApArtId) LEFT JOIN vFnsJahrespflanzen ON tblAktionsplan.ApArtId = vFnsJahrespflanzen.SisfNr) INNER JOIN ((tblApJBer INNER JOIN vApAnzMassnBisJahr ON tblApJBer.ApArtId = vApAnzMassnBisJahr.ApArtId) INNER JOIN tblKonstanten ON tblApJBer.ApBerJahr = tblKonstanten.ApBerJahr) ON tblAktionsplan.ApArtId = tblApJBer.ApArtId) LEFT JOIN ArtenDb_tblFloraFnsKef ON ArtenDb_tblFloraSisf.NR = ArtenDb_tblFloraFnsKef.SisfNr
...
SELECT tblApJBer.*, If(ArtenDb_tblFloraSisf.Deutsch is null,ArtenDb_tblFloraSisf.Name, CONCAT(ArtenDb_tblFloraSisf.Name, ' (', ArtenDb_tblFloraSisf.Deutsch, ')')) AS Artname, If(ArtenDb_tblFloraFnsKef.KefArt=-1,"Ja","") AS FnsKefArt2, If(Round((tblKonstanten.ApBerJahr-ArtenDb_tblFloraFnsKef.KefKontrolljahr)/4,0)=(tblKonstanten.ApBerJahr-ArtenDb_tblFloraFnsKef.KefKontrolljahr)/4,"Ja","") AS FnsKefKontrJahr2, vFnsJahrespflanzen.Jahr AS FnsJahrespflanze, vFnsJahrespflanzen.Jahresart AS FnsJahrespflanze2
FROM ((((ArtenDb_tblFloraSisf INNER JOIN (tblAktionsplan INNER JOIN vApApBerichtRelevant ON tblAktionsplan.ApArtId = vApApBerichtRelevant.ApArtId) ON ArtenDb_tblFloraSisf.NR = tblAktionsplan.ApArtId) LEFT JOIN vFnsJahrespflanzen ON tblAktionsplan.ApArtId = vFnsJahrespflanzen.SisfNr) LEFT JOIN ArtenDb_tblFloraFnsJahresarten ON ArtenDb_tblFloraSisf.NR = ArtenDb_tblFloraFnsJahresarten.SisfNr) LEFT JOIN ArtenDb_tblFloraFnsKef ON ArtenDb_tblFloraSisf.NR = ArtenDb_tblFloraFnsKef.SisfNr) INNER JOIN (((tblApJBer INNER JOIN vApAnzMassnBisJahr ON tblApJBer.ApArtId = vApAnzMassnBisJahr.ApArtId) INNER JOIN tblKonstanten ON tblApJBer.ApBerJahr = tblKonstanten.ApBerJahr) INNER JOIN tblKonstanten AS tblKonstanten_1 ON tblApJBer.ApBerJahr = tblKonstanten_1.ApBerJahr) ON tblAktionsplan.ApArtId = tblApJBer.ApArtId
...
SELECT tblApJBer.*, If(ArtenDb_tblFloraSisf.Deutsch is null,ArtenDb_tblFloraSisf.Name, CONCAT(ArtenDb_tblFloraSisf.Name, ' (', ArtenDb_tblFloraSisf.Deutsch, ')')) AS Artname, If(ArtenDb_tblFloraFnsKef.KefArt=-1,"Ja","") AS FnsKefArt2, If(Round((tblKonstanten.ApBerJahr-ArtenDb_tblFloraFnsKef.KefKontrolljahr)/4,0)=(tblKonstanten.ApBerJahr-ArtenDb_tblFloraFnsKef.KefKontrolljahr)/4,"Ja","") AS FnsKefKontrJahr2, vFnsJahrespflanzen.Jahr AS FnsJahrespflanze, vFnsJahrespflanzen.Jahresart AS FnsJahrespflanze2
FROM (((ArtenDb_tblFloraSisf INNER JOIN (tblAktionsplan INNER JOIN vApApBerichtRelevant ON tblAktionsplan.ApArtId = vApApBerichtRelevant.ApArtId) ON ArtenDb_tblFloraSisf.NR = tblAktionsplan.ApArtId) LEFT JOIN vFnsJahrespflanzen ON tblAktionsplan.ApArtId = vFnsJahrespflanzen.SisfNr) LEFT JOIN ArtenDb_tblFloraFnsKef ON ArtenDb_tblFloraSisf.NR = ArtenDb_tblFloraFnsKef.SisfNr) INNER JOIN ((tblApJBer INNER JOIN vApAnzMassnBisJahr ON tblApJBer.ApArtId = vApAnzMassnBisJahr.ApArtId) INNER JOIN tblKonstanten ON tblApJBer.ApBerJahr = tblKonstanten.ApBerJahr) ON tblAktionsplan.ApArtId = tblApJBer.ApArtId
